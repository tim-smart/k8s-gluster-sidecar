...
let main = (() => {
  var _ref = _asyncToGenerator(function* () {
    ...
    while (true) {
      const pods = yield k8s.getPodsThatMatchLabels(env.podLabels);
    ...
    while (true) {
      ...
      const peerIps = yield gluster.peerStatusIps();
      if (!peerIps.length) {
        continue;
      }
      const nonBrickIps = yield gluster.notYetBrickIps(env.volumeName, peerIps);
      const nonBricks = nonBrickIps.map(function (ip) {
        return `${ ip }:${ env.brickPath }`;
      });

      try {
        yield gluster.run('volume', 'add-brick', env.volumeName, ...nonBricks);
      } catch (err) {
        console.error(err.stack);
      }
    }
...
var readiness = _interopRequireWildcard(_readiness);
